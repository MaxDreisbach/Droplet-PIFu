...
def calc_error(opt, net, cuda, dataset, num_tests, plane_dim, ds='test', plot_results=False):
    ...
    with torch.no_grad():
        ...
        for idx in range(num_tests):
            ...
            if plot_results:
                ...
                plot_contour(opt, sample_tensor, res[0, 0, :], label_tensor[0, 0, :], plane_dim, 'alpha', 'alpha', name, ds)
                #plot_contour(opt, sample_tensor, res[0, 0, :], label_tensor[0, 0, :], 'y', 'alpha', 'alpha', name, ds)
                #plot_contour(opt, sample_tensor, res[0, 0, :], label_tensor[0, 0, :], 'z','alpha', 'alpha', name, ds)
                #plot_iso_surface(opt, sample_tensor, res[0, 0, :], label_tensor[0, 0, :], 'alpha', name, ds)
    ...
    print('All IOU: {0}'.format(IOU_arr))
