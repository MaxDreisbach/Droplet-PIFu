...
def train(opt):
    ...
    with torch.no_grad():
        ...
        val_errors = calc_error(opt, netG, cuda, dataset, len(plot_timesteps), SLICE_DIM, ds='test', plot_results=True)
        exit()
        ...
        train_errors = calc_error(opt, netG, cuda, dataset, len(plot_timesteps), ds='train', plot_results=True)
